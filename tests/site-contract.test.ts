...
describe('built public site contract', () => {
  ...
  it('ships security headers, a true 404 asset, social media assets, and raster icons', () => {
    ...
    expect(securityTxt).toContain('Policy: https://exif.utilitas.app/security/');
    const expiresAt = Date.parse(expiresFields[0]!.slice('Expires: '.length));
    expect(Number.isNaN(expiresAt)).toBe(false);
    expect(expiresAt).toBeGreaterThan(Date.now());
    expect(expiresAt).toBeLessThanOrEqual(Date.now() + 366 * 24 * 60 * 60 * 1000);
